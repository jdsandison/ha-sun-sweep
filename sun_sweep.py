...
def atrium_uplight_sunset(pos: float):
    if pos <= 85:
        return None
    t = (pos - 85) / 15.0
    if t <= 0.5:
...
def atrium_downlight_sunset(pos: float):
    if pos <= 85:
        return None
    t = (pos - 85) / 15.0
    if t <= 0.5:
...
def atrium_default_rgb(pos: float):
    k = tw_kelvin(pos)
    if k >= 5000:
        return (180, 200, 255)
    elif k >= 3500:
